...
type shift<'Size when 'Size :> INumber<'Size>> = (struct ('Size * 'Size))
...
module Coordinates =
    ...
    let getAfterShift ((i, j): shift<'Size>) ((x, y): coordinates<'Size>) : coordinates<'Size> =
        struct (x+i, y+j)
    
    /// Get a sequence of coordinates after applying shifts to given coordinates.
    let getAfterShifts (start: coordinates<'Size>) (shifts: (shift<'Size>) seq) : coordinates<'Size>seq =
    ...
    let getAfterShiftInAllDirections (start: coordinates<'Size>) ((i, j) : shift<'Size>) : coordinates<'Size>seq =
        [
            struct (i,j);
            struct (i,-j);
            struct (-i,j);
    ...
    let rec afterRepeatedShift (stopAt: coordinates<'Size> -> bool) (shift: shift<'Size>) (start: coordinates<'Size>) : coordinates<'Size>list =
